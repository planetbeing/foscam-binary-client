...
class FoscamEncryptionLayer extends FoscamPacketLayer {
    ...
    receivedPacket(type, packet) {
        ...
        if(type != 26)
            return [type, self.decrypt(packet)];
    ...
    decrypt(packet) {
        ...
        if(packet.length <= 128)
            return self.decrypt128(packet);

        // On large packets, Foscam encrypts the first 128 bytes of every 4096 byte block (!!).
        let left = packet.length;
        let offset = 0;
        while(left > 128) {
            self.decrypt128(packet.slice(offset, offset + 128)).copy(packet, offset);
            offset += 4096;
            left -= 4096;
        }
    ...
    sendPacket(type, payload) {
        let self = this;
        super.sendPacket(type, self.encrypt(payload));
    }
}
